...
AI_PROMPT_TEMPLATE: str = (
    ...
    "2. FORMAT RESPONSE: Respon keseluruhan HARUS berupa JSON murni yang valid. SANGAT PENTING: Jangan bungkus dengan markdown code block (```json). Jika membuat baris baru (newline) di dalam string untuk Markdown list/tabel, pastikan men-escape dengan benar (gunakan `\\n`) agar format JSON tidak rusak!\n"
    "3. GAYA PENULISAN (MARKDOWN): Anda SANGAT DISARANKAN menggunakan format Markdown (seperti **bold**, list poin-poin, atau tabel) DI DALAM string paragraf. DILARANG KERAS MENGGUNAKAN EMOJI agar bahasa tetap terlihat sangat profesional. Karena `paragraphs` adalah array, Anda juga bisa memisahkan paragraf/list baru sebagai elemen string terpisah di dalam array tersebut.\n"
