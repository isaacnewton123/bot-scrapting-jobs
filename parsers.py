"""
parsers.py — HTML parsing dan content cleaning.

Berisi:
- fetch_html()           : Download halaman web
- MyHTMLParser           : Ekstrak judul, konten, meta dari halaman loker BukaJobs
- ApplyPageParser        : Ekstrak link pendaftaran dari halaman /apply/
...
class MyHTMLParser(HTMLParser):
    """Parser untuk halaman detail loker BukaJobs.
    
    Mengekstrak:
    - title           : Judul lowongan (dari <h1 class="entry-title">)
    - content         : List teks konten artikel
    - apply_page_url  : URL halaman /apply/ jika ada
    ...
    def finalize(self) -> None:
        """Panggil setelah feed() untuk menangkap sisa konten terakhir."""
        if self.current_content.strip():
            self.content.append(self.current_content.strip())
            self.current_content = ""
...
class ApplyPageParser(HTMLParser):
    """Parser untuk halaman /apply/ yang berisi link pendaftaran."""
...
# Kata kunci yang menandakan akhir artikel (semua setelahnya = sampah)
...
# Kata kunci teks sampah yang harus dibuang (iklan, social, disclaimer)
...
def clean_and_structure_content(raw_content: list[str]) -> list[str]:
    ...
    Args:
        raw_content: List teks mentah dari HTML parser.
        
    Returns:
        List teks bersih yang siap dikirim ke AI.
    """
    ...
    for text in raw_content:
        ...
        # Skip baris setelah "Tanggal publikasi:"
        if skip_next:
            skip_next = False
            continue
